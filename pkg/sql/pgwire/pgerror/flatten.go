...
func Flatten(err error) *Error {
	...
	resErr.Hint = t.String()
	t.Reset()
	for _, d := range errors.GetAllDetails(err) {
		t.WriteString(d)
	}
	resErr.Detail = t.String()
...
func GetPGCode(err error) string {
	computeCode := func(err error) (code string, ok bool) {
		switch e := err.(type) {
		...
		case roachpb.ClientVisibleRetryError:
			return pgcode.SerializationFailure, true
		case roachpb.ClientVisibleAmbiguousError:
			return pgcode.StatementCompletionUnknown, true
		}
		// SimpleComputeDefault code already turns assertion errors into
		// code "Internal" and "unimplemented" errors into code
		// "FeatureNotSupported". We are relying on this behavior here.
		return errpgcode.SimpleComputeDefaultCode(err)
	}
	return errors.GetPGCodeEx(err, computeCode, errpgcode.SimpleCombineCodes)
}
